...
# seq
outs = infer_seq(1000, MODEL_SEQ, [0] + values)
assert outs == [1] + values

outs = infer_seq(1001, MODEL_SEQ, [100] + values2)
assert outs == [101] + values2

outs = infer_seq(str(uuid4()), MODEL_SEQ, [20] + values2)
assert outs == [21] + values2

# dyna
outs = infer_seq(1000, MODEL_DYN, [0] + values)
assert outs == [1] + [x + 1000 if x == 1 else x for x in values]

outs = infer_seq(1001, MODEL_DYN, [100] + values2)
assert outs == [101] + [-x + 1001 if x == 1 else -x for x in values]
print("PASS")
